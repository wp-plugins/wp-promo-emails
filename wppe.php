 ...
 */
  ...
  */
  ...
  define("WP_PROMO_EMAILS_RECORDS_TABLE", $wpdb->prefix . "wpperecords");
  ...
  function email_install()
{
  ...
  $wpdb->query("
     ...
     if(strtoupper($wpdb->get_var("show tables like '". WP_PROMO_EMAILS_RECORDS_TABLE . "'")) != strtoupper(WP_PROMO_EMAILS_RECORDS_TABLE))
    {
        $wpdb->query("
            CREATE TABLE IF NOT EXISTS `". WP_PROMO_EMAILS_RECORDS_TABLE . "` (
              `rid` int(11) NOT NULL auto_increment,
              `rsubject` text CHARACTER SET utf8 COLLATE utf8_bin NOT NULL ,
              `remail` char(250) NOT NULL ,
              `rsent` char(3) NOT NULL default '0',
               `ropen` char(3) NOT NULL default '0',
                `rbounce` char(3) NOT NULL default '0',
            ...
            CREATE TABLE `". WP_PROMO_EMAILS_NEWSLETTER_TABLE . "` (
              ...
              `nsent` char(3) NOT NULL default '0',
               `nopen` char(3) NOT NULL default '0',
                `nbounce` char(3) NOT NULL default '0',
  ...
  $registrant = $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->prefix}users");
  $recipient = $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->prefix}users WHERE subscriber='Y'");
  ...
  "
	SELECT user_email
	FROM {$wpdb->prefix}users WHERE subscriber='Y'"
  );
  foreach ( $team as $team )
 {
  $email = $team->user_email;
  $table_name2 = $wpdb->prefix . "wpperecords";
  $wpdb->insert( $table_name2, array( 'rsubject' => $_POST['subject_email'], 'remail' => $email, 'rsent' => 1 ) );
  ...
  $success ="<img src='$url/wp-content/plugins/wp-promo-emails/records/success.php?mail_1_1_1=$email=$author' width='0' height='0' >";
  $content = "$template<br>If you do not wish to receive this email please <a href='$unsubscribelink'>unsubscribe</a>.$success";
  $to = "$email";
  $subject = "$subject";
  $sender = "$name Support for you" ;
  $email = "$name<$admin>";
  ...
  foreach ( $resend as $resend )
 {
  ...
  if ( ! class_exists( 'WP_Promo_Emails' ) ) {
	...
	class WP_Promo_Emails {
      ...
      wp_enqueue_style('jPages', WPPE_CSS_URL . '/jPages.css');
      wp_enqueue_script( 'jPages', WPPE_JS_URL . '/jPages.js', array( 'jquery'), 'null', true );
      wp_enqueue_script( 'wppe-script', WPPE_JS_URL . '/wppe-script.js', array( 'jquery'), 'null', true );
		}
 ...
 */
  ...
  $contacts = $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->prefix}users " );
  if ($contacts < 1000) {
    $users = number_format($contacts);
  }elseif ($contacts < 1000000) {
    $users = number_format($contacts / 1000) . 'K';
   } else if ($contacts < 1000000000) {
    // Anything less than a billion
    $users = number_format($contacts / 1000000, 1) . 'M';
   } else {
    // At least a billion
    $users = number_format($contacts/ 1000000000, 1) . 'B';
   }
  ...
  echo "<div id='wppe_carton' style='width: 260px;height: 175px;border:solid 1px #999;border-radius: 3px;margin-bottom:10px;'>";
  echo "<div class='wppe_carton-top' style='border-bottom:1px solid #999;width:260px;text-align:center;font-size:14px;line-height:16px;color:#EEE;
        height: 45px;padding-top: 10px;text-transform:uppercase;background:#444;'>Newsletter<br><span style='padding-top: 5px;font-size: 18px;color: #FFF;
        float: left;text-align: center;width: 260px;'>";
  echo $promo;
  echo "</span></div>";
  echo "<div class='wppe_carton-left' style=' border-right:1px solid #999;width: 129px;text-align: center;font-size: 14px;line-height: 16px;
        color: #999;height: 89px;padding-top: 30px;text-transform: uppercase;background: #EEE;'>Members<br><span style='padding-top: 5px;font-size: 18px;
        color: #666;float: left;text-align: center;width: 130px;'>";
  echo $users;
  echo "</span></div>";
  echo "<div class='wppe_carton-right' style='width: 129px;text-align: center;font-size: 14px;line-height: 16px;color: #999;height: 89px;padding-top: 30px;
        text-transform: uppercase;background: #EEE;float: right;margin-top: -119px;'>Subscribers<br><span style='padding-top: 5px;font-size: 18px;color: #666;
