...
Plugin URI: http://cbfreeman.com/wp-promo-emails-premier/
  ...
  function email_install()
{
    ...
    $wpdb->query(
	"
	UPDATE `". WP_PROMO_EMAILS_NEWSLETTER_TABLE . "`
	SET nopen = 0
	"
    );
  ...
  $admin = get_option( 'admin_email' );
	...
	// Send Preview Email
	if(isset($_POST['wppe_send_preview'])) {
  ...
  foreach ( $team as $team )
 {
  ...
  $success = "<img src=" . get_option('siteurl') . "/wp-content/plugins/wp-promo-emails/records/success.php?mail_1_1_1=$email=$author>";
 ...
 */
  ...
  global $wpdb;
  $promo=$wpdb->get_var("SELECT COUNT(nid) FROM {$wpdb->prefix}wppenewsletter" );
  $users = $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->prefix}users" );
  $subs = $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->prefix}users WHERE subscriber='Y'");
